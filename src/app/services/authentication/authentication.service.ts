          ...
          .then((document) => {
            if(data.sourcePage === 'detailsPage') {
              ...
              const recapData = {
                ...
                price : data.trainingPrice,
                userID : usr.user.uid
              };
              router.navigate(['/recap',recapData]);
            } else if (data.sourcePage === 'homePage') {
              const allData = {userID : usr.user.uid, userFullName : document.data().fullName};
              router.navigate(['/purchased-trainings', allData]);
            }
          });
        } else {
    ...
    if (password === confirmPassword) {
      ...
      .then((usr) => {
        setDoc(doc(db, 'users', usr.user.uid), {
          ...
          const recapData = {fullName : user.fullname, training : data.trainingName, price : data.trainingPrice, userID : usr.user.uid};
          router.navigate(['/recap',recapData]);})
        .catch(() => console.log('Failed to create doc'));
      })
      .catch((error) => alert('All fields are required'));
    } else {
